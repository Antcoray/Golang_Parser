...
async function uploadFile(file) {
    ...
    try {
        const response = await fetch('/uploadflow', {
            method: 'POST',
            body: formData
        });
...
function buildCombinedTable(data) {
    ...
    const rowsCount = Math.max(operators.length);
...
function updateExtendedMetrics(data) {
    ...
    document.getElementById('dict-n').innerHTML = `Максимальный уровень вложенности условного оператора CLI: ${CLI}`;
    document.getElementById('length-N').innerHTML = `Абсолютная сложность CL: ${CL}`;
    document.getElementById('volume-V').innerHTML = `Относительная сложность cl: ${cl}`;
}
